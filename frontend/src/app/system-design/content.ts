...
// Using String.raw to avoid template literal escaping issues with markdown backticks

export const ARCHITECTURE_CONTENT: string = [
  ...
  "```mermaid",
  "graph TD",
  '    A["API Routes<br/>/api/v1/*"] --> B["Services<br/><i>Business Logic</i>"]',
  ...
  "|---|---|---|",
  "| `health.py` | `GET /health`, `GET /health/detailed` | Liveness + readiness probes with subsystem status |",
  "| `documents.py` | `POST /documents/upload`, `GET /documents`, `GET /documents/{id}` | PDF upload, document listing, detail fetch |",
  "| `query.py` | `POST /query`, `POST /query/stream`, `GET /query/{id}/replay` | Synchronous query, SSE streaming, query replay |",
  ...
  "|---|---|",
  "| **IngestionService** | Full upload pipeline: validate → parse PDF → chunk → embed → index → persist |",
  "| **QueryService** | Full query pipeline: intent → retrieve → rerank → compress → generate → score |",
  "| **ChunkingService** | Semantic-aware text chunking with overlap and page tracking |",
  "| **EmbeddingService** | Sentence-transformer embedding generation for chunks and queries |",
  "| **RetrievalService** | Hybrid retrieval (vector + BM25) with alpha-weighted score fusion |",
  "| **RerankingService** | Cross-encoder neural reranking of retrieved chunks |",
  "| **ConfidenceService** | Multi-factor confidence scoring (similarity + reranker + agreement + dispersion) |",
  "| **IntentService** | Query intent classification (factual, analytical, comparative, summarization) |",
  "",
  "---",
  "",
  ...
  "|---|---|---|",
  "| **Database** | SQLAlchemy 2.0 + asyncpg | Async PostgreSQL connection pool with session factory |",
  "| **Vector Store** | FAISS | Dense vector index for semantic similarity search |",
  "| **BM25 Store** | rank-bm25 | Sparse keyword index for lexical matching |",
  "| **LLM Provider** | OpenAI API / Mock | Abstraction over LLM providers with streaming support |",
  "",
  "---",
  "",
  ...
  "| `api.ts` | Centralized API client with typed generics, SSE streaming |",
  ...
  "    IS->>CS: chunk_pages(pages)",
  "",
  "    Note over IS: Stage 5 — Embedding",
  "    IS->>ES: embed_chunks(chunks)",
  "",
  "    Note over IS: Stage 6 — Vector Indexing",
  "    IS->>VS: add_vectors(embeddings)",
  "",
  "    Note over IS: Stage 7 — BM25 Indexing",
  "    IS->>BM: add_documents(chunks)",
  "",
  "    Note over IS: Stage 8 — Chunk Persistence",
  "    IS->>DB: Bulk insert Chunk records",
  "",
  "    IS->>DB: Update IngestionJob completed",
  ...
  "    QS->>CS: score(components)",
  "    CS-->>QS: ConfidenceBreakdown",
  "",
  "    Note over QS: Stage 9 — Audit Logging",
  "    QS->>DB: Create QueryLog",
  "",
  "    QS-->>API: QueryResponse",
  "    API-->>FE: Full response with citations",
  "    FE-->>U: Render answer + transparency",
  "```",
  "",
